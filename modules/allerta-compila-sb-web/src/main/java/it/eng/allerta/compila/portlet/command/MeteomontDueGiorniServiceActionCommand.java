...
@Component(
		...
		property = {
				...
				"mvc.command.name=/allertaer/allerta_valanghe/webservice2"
		},
		service = MVCActionCommand.class
	)
public class MeteomontDueGiorniServiceActionCommand extends BaseMVCActionCommand {

	@Override
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		
		AllertaValangheBean avb = new AllertaValangheBean(PortalUtil.getHttpServletRequest( actionRequest));
		String output = avb.creaDaWebServiceDueGiorni();
		if ("".equals(output)) {
			//successo
