...
public class CaricaDBTask extends BaseDispatchTaskExecutor {
	...
	public void doExecute(DispatchTrigger dispatchTrigger, DispatchTaskExecutorOutput output){
		...
		try {
			...
			if (!prosegui) {
				logger.info("Scheduler gi� in esecuzione, esco");
	...
	public void aggiornaTuttiValori() {
		...
		try {	
			...
			if (aggiornaValoriSuperficiale("254,0,0/103,2000,-,-/B13003","umidit�",stringa))
				aggiornato = true;
...
*/			
	...
	public void regoleAllarme() {
		...
		try {
			...
			//elenco di tutte le condizioni superate; se una regola non compare qui, non pu� essere superata
				...
				//try {
					...
					if (d != 0.0) {
						...
						if (condizionePrincipale) {
							//regola NON soddisfatta ma lo � la condizione principale:
							//vuol dire che un idrometro � sopra soglia ma nessuno dei
							//suoi pluviometri lo �.
							try {
								...
								if (l!=null && l.size()==0) { //evento non ancora segnalato?
									...
									//notifica, ma solo se la regola non � scattata da poco (creerebbe confusione).
			...
			for (RegolaAllarme ra : regoleSoddisfatte) {
				...
				if (!trovato) {
					...
					//allarme attivo, ma la sua regola non � tra quelle soddisfatte,
					//quindi deve cessare.
	 ...
	 * non si � in corso di allerta idrogeologica. In questo caso usa una
	...
	public void controlloPioggiaMonitoraggio() {
		...
		try {
			...
			if (accesa!=null && accesa.getValore()!=null && accesa.getValore().equals("true")) {
				//20240513 se il monitoraggio � acceso E siamo in verde idro-geo-temporali E questo � il documento pi� recente,
				...
				if (ora>0 || minuto>=10) {
					//esegui il controllo solo verso mezzanotte, cos� da non impedire
				...
				if (i.intValue()<1) {
					...
					try {
						...
						String testoMail = "Si comunica lo spegnimento automatico della mappa di monitoraggio in quanto non ci sono allerte idro-geo-temporali in corso di validit� presente o futura.";
