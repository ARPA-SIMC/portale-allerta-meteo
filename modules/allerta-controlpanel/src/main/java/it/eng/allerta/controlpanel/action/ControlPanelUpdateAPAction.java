...
@Component(
		...
		property = {
				...
				"mvc.command.name=/controlpanel/ap/update"
		},
		service = MVCActionCommand.class
		)
public class ControlPanelUpdateAPAction extends BaseMVCActionCommand{
	...
	protected void doProcessAction(ActionRequest actionRequest, ActionResponse actionResponse) throws Exception {
		...
		try {
			HttpServletRequest httpRequest = PortalUtil.getOriginalServletRequest( 
					r); 
			String valore = ParamUtil.getString(httpRequest, "ap_" + id);
			if (valore==null) {
				SessionErrors.add(actionRequest, "Errore", "Parametro "+id+" non trovato.");
				return;
			}
			AllertaParametro ap = AllertaParametroLocalServiceUtil.fetchAllertaParametro(id);
			if (ap==null) {
				SessionErrors.add(actionRequest, "Errore", "Parametro "+id+" non esiste.");
				return;
			}
			ap.setValore(valore);
			AllertaParametroLocalServiceUtil.updateAllertaParametro(ap);
			...
			LogInternoLocalServiceUtil.log("control panel", "ControlPanelUpdateAPAction", e, "");
