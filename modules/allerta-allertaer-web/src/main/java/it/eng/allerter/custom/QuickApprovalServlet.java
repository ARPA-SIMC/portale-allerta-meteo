...
public class QuickApprovalServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String allerta = req.getParameter("id");
		String status = req.getParameter("stato");
		
		int stato = (status!=null?Integer.parseInt(status):0);
		Map<String, Serializable> map = new HashMap<String, Serializable>();
		
		map.put(WorkflowConstants.CONTEXT_ENTRY_CLASS_PK, Long.parseLong(allerta));
	
		try {
			updateStatus(stato,map);
		} catch (Exception e) {
			logger.error(e);
		}
	}
	...
	public Allerta updateStatus(int status, Map<String, Serializable> workflowContext2)
		...
		System.out.println("ALLERTA WORKFLOW (PORTLET) " + status);

		//if (pubblicato == null || revisione == null || lavorazione == null)
			//caricaPubblicatoTag();
  
		long resourcePrimKey = GetterUtil.getLong(workflowContext2.get(WorkflowConstants.CONTEXT_ENTRY_CLASS_PK));
		...
		if (status == WorkflowConstants.STATUS_PENDING && feedback.getStato() == WorkflowConstants.STATUS_DENIED) {
			 ...
			 * getNumero()
			 * +" � in attesa di approvazione: https://allertameteo.regione.emilia-romagna.it"
			 ...
			 * ; subject =
			 * "Il documento "+feedback.getNumero()+" � in attesa di approvazione";
			 * 
				...
				if (status != WorkflowConstants.STATUS_DENIED) {
					...
					mandaNotifica(feedback.getUtenteFirmaProtId(),
							"Il documento " + feedback.getNumero()
									+ " � in attesa di approvazione su https://allertameteo.regione.emilia-romagna.it",
							feedback, tipo, sottotipo, l);

					text = "<html><head></head><body>Il documento " + feedback.getNumero()
							+ " e' in attesa di approvazione su <a href=\"https://allertameteo.regione.emilia-romagna.it\">https://allertameteo.regione.emilia-romagna.it</a></body></html>";
					subject = "Il documento " + feedback.getNumero() + " � in attesa di approvazione";
