...
public class SuperamentiPluvioFiller extends QueryBasedColumnFiller{

	static String query = "select string_agg(to_char(x.createdate at time zone 'utc' " + 
			"at time zone 'europe/rome','HH24:MI') || ' ' || x.name || ' ' || coalesce(x.livello,''), ', ') as risultato from (select v.createdate,v.name,v.soglia,v.livello,v.sogliaregola," + 
			...
			"where tipo='pluvio' and zona=%ZONA%" + 
	...
	public SuperamentiPluvioFiller() {
		super(query);
	}
