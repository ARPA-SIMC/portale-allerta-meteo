...
<head>
   ...
   <script data-senna-track="permanent" src="${javascript_folder}/jquery-3.7.1.min.js"></script>
...
<div class="am am-${w3c_language_id}" >
   ...
   function cleanLanguageMenu() {
      ...
      if(langlist){
           ...
           for(var i=0; i<langlist.length; i++) { 
                ...
                if (langtxt=='english-United Kingdom [beta]') {
		 langlist.item(i).innerText='English';
		<#if !is_signed_in >
		langlist.item(i).parentNode.href = window.location.href.replace(".emilia-romagna.it/",".emilia-romagna.it/en/");
		 </#if>
		}
                if (langtxt=='italiano-Italia [beta]') {
			langlist.item(i).innerText='Italiano';
		<#if !is_signed_in >
		langlist.item(i).parentNode.href = window.location.href.replace(".emilia-romagna.it/en/",".emilia-romagna.it/");
		  </#if>
		}
           }
      }
