package it.eng.previsioni.scheduler;
...
import it.eng.bollettino.service.ImgLocalServiceUtil;
...
public class BollettinoScheduler extends BaseMessageListener {
	...
	protected void activate(Map<String,Object> properties) throws SchedulerException {
		...
		String className = (BollettinoScheduler.class.getName();
	...
	private void getXml(){
		...
		_log.debug("Cron Expression BollettinoScheduler : getXml");
		...
		try{
			for(String giorno:giorni){
				for(String parte:parti){
					...
					img.setData(Base64.encodeBase64String(IOUtils.toByteArray(stream)));
					ImgLocalServiceUtil.updateImg(img);
	...
	private Img getImg(String parte, String giorno, String provincia) throws SystemException {
		Img img = null;
		try {
			img = ImgLocalServiceUtil.getByPartDayProvincia(parte, giorno, provincia);
		}catch (NoSuchImgException e) {
			img = ImgLocalServiceUtil.createImg(CounterLocalServiceUtil.increment());
	...
	private Bollettino getBollettino(String type){
		Bollettino bollettino = null;
		bollettino = BollettinoLocalServiceUtil.findByTipo(type);
		if (bollettino ==null){
			try {
				bollettino = BollettinoLocalServiceUtil.createBollettino(CounterLocalServiceUtil.increment());
